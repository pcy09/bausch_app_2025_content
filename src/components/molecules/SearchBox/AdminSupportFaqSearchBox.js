...
const AdminSupportFaqSearchBox = ({ showStatusOption, onHandleSearchData, categoryOptions }) => {
  ...
  const handleSearch = data => {
    const { date, searchType, searchText, searchInputText, selectOption, retiredProductOptions } = data;
    const searchData = {
      ...
      retiredProductOptions: retiredProductOptions ?? null,
    ...
    <CardContainer css={[marginBottomStyle(24)]}>
      <Form onSubmit={handleSubmit(onSubmit, onError)}>
        <FilterBox handleReset={handleResetSearch}>
          <DateSearchAtom title={'FAQ 등록일'} control={control} />
          <SelectAtom defaultValue="all" name="show_status" control={control} title={'노출 여부'} options={showStatusOption} />
          <SelectAtom defaultValue="all" name="category" control={control} title={'카테고리'} options={categoryOptions} />
        </FilterBox>
      </Form>
    </CardContainer>
  );
...
export default AdminSupportFaqSearchBox;
