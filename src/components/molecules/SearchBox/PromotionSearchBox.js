...
const PromotionSearchBox = ({ selectOptions, onHandleSearchData }) => {
    ...
    <CardContainer css={[marginBottomStyle(24)]}>
      <Form onSubmit={handleSubmit(onSubmit, onError)}>
        <FilterBox handleReset={handleResetSearch}>
          <DateSearchAtom title={'프로모션 기간'} control={control} />
          <SelectAtom defaultValue="all" control={control} title={'노출 여부'} options={selectOptions} />
        </FilterBox>
      </Form>
    </CardContainer>
  );
