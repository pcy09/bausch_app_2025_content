...
const StoreDeleteExcelFormSection = ({ setResult, result, skuFileList, setSkuFileList, getExcelData }) => {
  const columns = [
    ...
    <>
      <CardContainer>
        <Descriptions
          ...
          <Descriptions.Item span={3} label="스토어 제외">
            <Upload accept=".xlsx" maxCount={1} onChange={handleFileChange} fileList={skuFileList}>
              <Buttons type={'dashed'} name={'삭제 파일 업로드'} icon={<UploadOutlined />} />
            </Upload>
          </Descriptions.Item>
        </Descriptions>
      </CardContainer>
...
export default StoreDeleteExcelFormSection;
