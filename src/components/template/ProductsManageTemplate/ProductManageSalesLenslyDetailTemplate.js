...
export default function ProductManageSalesLenslyDetailTemplate() {
  const dispatch = useDispatch();
  const router = useRouter();

  const { productGroupLensly, lensCycle, lensPowerType, storeGroup } = useCommonCodeBatch([
    'productGroupLensly',
    'lensCycle',
    'lensPowerType',
    'storeGroup',
  ...
  // 제품 상세 데이터 불러오기
  const productDetail = useSelector(state => state.productLensly.productDetail);
  ...
  const skuHandler = file => {
    ...
    dispatch(updateLenslySkuAction({ formData }));
  };
  // 데이터 넣어주는 함수
        ...
        const transformedProductPointPriceGroupModels = productPointPriceGroupModels.map(item => {
          return {
            ...
            storeGroupId: item.storeGroupId.map(id => {
              ...
              return [matchedGroup?.label ? matchedGroup.value : ''];
            }),
          };
        });
  ...
  useEffect(() => {
    if (!isSkuUploadingRef.current) {
      setProductDetailData();
    }
  }, [productDetail, productGroupBauschType, setValue, storeGroupType]);
  ...
  const uploadProps = {
    onRemove: file => {
      setSkuFileList([]);
      dispatch(updateInsertLenslySkuAction({ skuInfoModels: [] }));
    },
    beforeUpload: file => {
      return false;
    },
  ...
  const handleSendData = data => {
    ...
    if (productDetail.skuInfoModels.length === 0) {
      ...
      dispatch(updateProductLenslyDetailAction({ id, sendObject, callback: router }));
    }
  };
  ...
  const removePurchaseSetting = id => {
    ...
    // setPurchaseSettings(purchaseSettings.filter(setting => setting.id !== id));
  };
  ...
  useEffect(() => {
    if (router?.query?.id) {
      dispatch(getProductLenslyDetailAction({ id: router.query.id, callback: router }));
    }
  ...
  useEffect(() => {
    ...
    if (productGroupLensly && productGroupLensly.length > 0) {
      setProductGroupBauschType(transFormRadioValue(productGroupLensly));
    }
    // setValue('productPointPriceGroupModels', [{ id: 1, storeGroupId: '', pointPrice: '' }]);
  }, [lensPowerType, productGroupLensly, storeGroup]);
  ...
  return (
    <>
      <div css={descStyle}>
        <NoticeLabel title={'👉🏼 렌즐리의 판매 제품상세 페이지입니다. 렌즐리 제품을 확인 및 수정할 수 있습니다.'} />
      </div>
          ...
          <Descriptions title={'제품 정보'} labelStyle={{ width: '250px' }} bordered={true} column={4}>
            ...
            <Descriptions.Item span={2} label="제품 그룹⭐️">
              <Controller
                ...
                rules={{ required: true }}
                render={({ field: { ref, value, ...rest }, fieldState }) => {
                  return <SelectBox value={value || []} options={productGroupLensly} placeholder={'선택해주세요'} {...rest} />;
                }}
              />
            </Descriptions.Item>
          ...
          <RowGrid>
            <ColGrid span={23} css={buttonFlexStartRowStyle}>
              <Buttons
                ...
                onClick={() => router.push('/admin/product/manage?tab=L')}
              />
            </ColGrid>
